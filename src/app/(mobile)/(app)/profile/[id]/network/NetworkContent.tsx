...
export default function NetworkContent({ params }: { params: Promise<{ id: string }> }) {
  const resolvedParams = use(params)
  const router = useRouter()
  const searchParams = useSearchParams()
  const initialTab = (searchParams.get('tab') || 'followers') as 'followers' | 'following'

  const [activeTab, setActiveTab] = useState<'followers' | 'following'>(initialTab)
  const [list, setList] = useState<NetworkUser[]>([])
    ...
    async function loadData() {
      ...
      try {
        const res = await fetch(
          `/api/profile/${resolvedParams.id}/network?type=${activeTab}`
        )
        if (res.ok) {
    ...
    <div className="min-h-screen bg-gray-50 font-[family-name:var(--font-dm)]">
      ...
      <div className="flex bg-white px-5 border-b border-gray-100">
        ...
        >
          Seguidores
          {activeTab === 'followers' && (
            <div className="absolute bottom-0 left-4 right-4 h-1 bg-[#E05300] rounded-t-full" />
          )}
        </button>
        <button
