...
export default function PublicProfilePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    async function load() {
      ...
      try {
        const res = await fetch(`/api/profile/${resolvedParams.id}`, { headers: { Authorization: `Bearer ${session.access_token}` } })
    ...
    <div className="min-h-screen bg-gray-50 font-[family-name:var(--font-dm)] pb-24 relative">
      ...
      <div className="relative overflow-hidden px-6 pt-12 pb-16" style={{ background: 'linear-gradient(160deg, #830200 0%, #E05300 55%, #FF8C00 100%)' }}>
        ...
        <div className="relative z-10 flex items-center justify-between gap-4">
          ...
          {/* Botão Seguir (Oculto se for o próprio perfil) */}
          {!profile.isMe && (
            <button 
                ...
                className={`px-5 py-2 rounded-xl text-sm font-bold shadow-md transition-all active:scale-95 ${
                    profile.isFollowing 
                    ? 'bg-white/20 text-white border border-white/30 backdrop-blur-md' // Estilo "A Seguir"
                    : 'bg-white text-[#E05300]' // Estilo "Seguir"
                }`}
            >
