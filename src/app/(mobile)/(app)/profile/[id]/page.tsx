  ...
  return (
    <div className="min-h-screen bg-gray-50 font-[family-name:var(--font-dm)] pb-24 relative">
      ...
      {isPhotoViewerOpen && selectedPhoto && (
        ...
        >
          ...
          <img
            src={selectedPhoto}
            alt="Visualização"
            className="max-w-[95%] max-h-[85%] object-contain rounded-3xl shadow-2xl shadow-orange-500/10 border-2 border-white/10"
          />
        </div>
      )}
      ...
      >
        <button
          onClick={() => router.back()}
          className="absolute top-12 left-6 z-10 w-10 h-10 rounded-2xl bg-white/20 flex items-center justify-center text-white backdrop-blur-md border border-white/30 active:scale-90 transition-transform"
        >
          <svg
            width="20"
            height="20"
        ...
        <div className="relative z-10 flex flex-col items-center text-center mt-6">
          <div className="relative mb-4">
            <img
              ...
              alt="Avatar"
              className="w-24 h-24 rounded-[32px] object-cover border-4 border-white/30 shadow-2xl"
            />
          </div>
          <h1 className="text-white font-black text-2xl leading-tight drop-shadow-md">
            {profile?.displayName}
          </h1>
          ...
          <button
            ...
            className={`mt-6 px-10 py-3 rounded-[20px] font-black text-xs transition-all active:scale-95 shadow-xl ${
              profile?.isFollowing
                ? "bg-white/10 text-white border border-white/40 backdrop-blur-md"
        ...
        <div className="relative z-10 flex gap-3 mt-10">
          <div
            className="flex-1 text-center rounded-[24px] py-4 backdrop-blur-md border border-white/10"
            style={{ background: "rgba(255,255,255,0.12)" }}
          >
            <p className="text-white font-black text-xl leading-none">
              {profile?.completedRoutes?.length ?? 0}
            </p>
            <p className="text-white/60 text-[9px] font-black uppercase mt-1.5 tracking-widest">
              Rotas
            </p>
          </div>
          <Link
            href={`/profile/${profile?.id}/network?tab=followers`}
            className="flex-1 text-center rounded-[24px] py-4 backdrop-blur-md border border-white/10 active:scale-95 transition-transform"
            style={{ background: "rgba(255,255,255,0.12)" }}
          >
            <p className="text-white font-black text-xl leading-none">
      ...
      <div className="flex mx-6 mt-2 rounded-[22px] overflow-hidden border border-gray-100 bg-white mb-6 shadow-sm p-1">
        {(["routes", "badges"] as const).map((tab) => (
          <button
            ...
            className={`flex-1 py-3 text-[10px] font-black transition-all rounded-[18px] ${
              activeTab === tab
                ...
                return (
                  ...
                  >
                    <div className="px-6 pt-5 pb-4 flex justify-between items-start border-b border-gray-50">
                      <div>
                        <div
                          className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg mb-2 ${info.color}`}
                        >
                        ...
                        <h3 className="font-black text-gray-900 text-lg leading-tight tracking-tight">
                          {route.routeName}
                        </h3>
                        <p className="text-gray-400 text-xs mt-1.5 font-bold">
                      ...
                      <Link
                        ...
                        className="text-[10px] font-black text-orange-600 bg-orange-50 px-3 py-1.5 rounded-xl border border-orange-100 active:scale-90 transition-all"
                      >
                        VER MAPA
                      </Link>
                    </div>
                    ...
                    <div className="px-6 py-5">
                      <p className="text-gray-400 text-[9px] font-black uppercase tracking-widest mb-3">
                        Registros da Aventura
                      </p>
                        ...
                        <div className="flex gap-3 overflow-x-auto scrollbar-hide pb-2 px-1">
                          {route.photos.map((p, i) => (
                            ...
                            >
                              <img
                                src={p}
                                className="w-full h-full rounded-[24px] object-cover border-2 border-white shadow-md ring-1 ring-gray-100"
                              />
                            </button>
                          ))}
                        </div>
              ...
              <div className="py-24 text-center flex flex-col items-center gap-5">
                ...
                <div>
                  <p className="text-gray-900 font-black text-base">
                    Nenhuma rota oficial
                  </p>
                  <p className="text-gray-400 text-xs font-medium px-10 mt-1">
                    Este aventureiro ainda não completou rotas oficiais no Giro.
                  </p>
                </div>
              </div>
            )}
          ...
          <div className="grid grid-cols-3 gap-4">
            {profile?.badges && profile.badges.length > 0 ? (
              profile.badges.map((b) => (
                <div
                  key={b.id}
                  className="bg-white rounded-[28px] p-4 text-center shadow-sm border border-gray-50 flex flex-col items-center justify-center gap-2"
                >
                  ...
                  <p className="text-[10px] font-black text-gray-900 leading-tight uppercase line-clamp-2 tracking-tighter">
                    {b.name}
                  </p>
                </div>
              ))
