...
export default function ExploreMapPage() {
  ...
  useEffect(() => {
    ...
    import('leaflet').then((L) => {
      if (!mapRef.current) {
        ...
        }).setView([parseFloat(routes[0].startLatitude!), parseFloat(routes[0].startLongitude!)], 14)
      ...
      routes.forEach(route => {
        ...
        marker.on('click', () => {
          ...
          if (card && carouselRef.current) {
            ...
            const scrollPos = card.offsetLeft - (window.innerWidth / 2) + (card.offsetWidth / 2)
      ...
      if (activeRouteId && markersRef.current[activeRouteId]) {
        ...
        if (route?.startLatitude) {
          mapRef.current.flyTo([parseFloat(route.startLatitude), parseFloat(route.startLongitude!)], 15)
        }
      }
    })
  ...
  return (
    <div className="relative w-full h-screen bg-gray-100 overflow-hidden font-[family-name:var(--font-dm)]">
      ...
      <div ref={mapContainerRef} className="absolute inset-0 z-0 bg-[#f8f9fa]" />

      <div className="absolute top-12 left-6 right-6 z-[400] flex items-center justify-between pointer-events-none">
        <button onClick={() => router.back()} className="w-12 h-12 rounded-2xl bg-white/95 shadow-xl flex items-center justify-center border border-gray-100 pointer-events-auto active:scale-90 transition-transform">
        ...
        <div className="bg-white/95 backdrop-blur-md px-5 py-3 rounded-full shadow-lg border border-gray-100 pointer-events-auto">
          <p className="text-[10px] font-black text-gray-900 tracking-widest uppercase">{routes.length} Ativas no Giro</p>
        </div>
      </div>

      <div className="absolute bottom-8 left-0 right-0 z-[400]">
        {loading ? (
          <div className="flex justify-center p-8"><div className="w-10 h-10 border-4 border-orange-600 border-t-transparent rounded-full animate-spin shadow-lg" /></div>
        ) : (
          <div ref={carouselRef} onScroll={handleScroll} className="flex gap-4 overflow-x-auto px-6 pb-6 pt-4 snap-x snap-mandatory scrollbar-hide">
            {routes.map(route => (
              <div key={route.id} id={`card-${route.id}`} className="snap-center shrink-0 w-[85vw]" style={{ transform: activeRouteId === route.id ? 'scale(1)' : 'scale(0.95)', opacity: activeRouteId === route.id ? 1 : 0.6 }}>
                <div className="pointer-events-auto shadow-2xl rounded-[32px] bg-white ring-4 ring-transparent transition-all" style={{ borderColor: activeRouteId === route.id ? '#E05300' : 'transparent' }}>
                  <RouteCard {...route} />
                </div>
              </div>
            ))}
