...
export default function ShareClient() {
  ...
  const { activityType, coordinates, startTime, pausedDuration, distanceKm } =
    store;
  ...
  async function saveActivityToDb(imageBase64: string | null = null) {
    ...
    try {
      ...
      if (error.name === "AbortError") {
        console.warn(
          "Timeout offline alcançado na exportação. Prosseguindo localmente."
        );
      ...
      alert(
        "Ops, não conseguimos salvar seu treino na nuvem. Verifique sua conexão."
      );
      return false;
    } finally {
      setIsSaving(false);
    ...
    try {
      ...
      if (Capacitor.isNativePlatform()) {
        ...
        await Share.share({
          ...
          text: `Completei ${distanceKm.toFixed(2)} km de ${
            meta?.label
          } no GIRO! 🔥`,
          files: [uri],
        });
  ...
  return (
    ...
    >
      <div className="px-5 pt-12 pb-4 flex items-center justify-between">
        <div>
          <p className="text-white/40 text-xs uppercase tracking-widest font-bold mb-0.5">
            Criar Post
          </p>
        ...
        >
          {bgPhoto ? (
            ...
            <div
              ...
              style={{
                background:
                  "linear-gradient(160deg, #1A0800 0%, #0A0A0A 50%, #001A08 100%)",
              }}
            />
          )}
