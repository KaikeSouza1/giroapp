...
export default function FeedPage() {
  ...
  const [activeCommentSession, setActiveCommentSession] = useState<
    string | null
  >(null);
  ...
  async function toggleLike(sessionId: string, currentLiked: boolean) {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(
          `Status ${res.status}: ${body?.error ?? "Erro desconhecido"}`
        );
      }
    ...
    <div className="min-h-screen bg-gray-50 font-[family-name:var(--font-dm)] pb-24 relative">
      ...
      {activeCommentSession && (
        <div
          ...
          onClick={() => setActiveCommentSession(null)}
        >
          <div
            className="bg-white w-full h-[75vh] rounded-t-[32px] flex flex-col shadow-2xl animate-in slide-in-from-bottom-full duration-300"
              ...
              >
                <div className="flex items-center gap-4 p-4 hover:bg-orange-50 border-b border-gray-50 last:border-0 cursor-pointer active:bg-orange-100 transition-colors">
                  <img
                    src={user.avatarUrl || ""}
                    alt=""
                    className="w-12 h-12 rounded-2xl object-cover border-2 border-gray-100 shadow-sm"
                  />
                  <div>
                    <p className="font-black text-gray-900 text-sm leading-none mb-1">
                      {user.displayName}
                    </p>
              ...
              >
                  ...
                  <div className="flex items-center gap-3 px-5 py-4">
                    <div className="relative">
                      <img
                        ...
                        className="w-11 h-11 rounded-2xl object-cover border-2 border-orange-100 shadow-sm"
                      />
                      <div className="absolute -bottom-1 -right-1 w-4 h-4 bg-green-500 border-2 border-white rounded-full" />
                    </div>
                    <div className="flex-1">
                      <p className="font-black text-gray-900 text-[13px] leading-tight">
                        {item.userName}
                      </p>
                      <p className="text-gray-400 text-[11px] font-medium tracking-tight">
                        @{item.userUsername} • {timeAgo(item.completedAt)}
                      </p>
                    </div>
                ...
                <div className="flex items-center gap-6 px-6 py-4 bg-gray-50/50 border-b border-gray-50">
                  <div className="flex flex-col">
                    <span className="text-gray-400 text-[9px] uppercase font-black tracking-widest mb-0.5">
                      Distância
                    </span>
                  ...
                  >
                    ...
                    <span
                      className={`text-sm font-black ${
                        item.hasLiked ? "text-red-500" : "text-gray-500"
                      }`}
                    >
                      {item.likesCount}
                    </span>
