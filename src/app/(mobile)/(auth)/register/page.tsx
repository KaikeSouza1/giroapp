...
export default function RegisterPage() {
  ...
  const inputClass =
    "w-full px-4 py-3.5 rounded-xl text-sm text-gray-800 placeholder-gray-400 outline-none transition-all duration-200";
  ...
  async function openCapacitorCamera(source: "camera" | "gallery") {
    ...
    try {
      ...
      if (!isCancelled) {
        setError(
          "Erro ao acessar câmera. Verifique as permissões nas configurações."
        );
      }
    } finally {
  ...
  async function handleFinalSubmit() {
    ...
    try {
      ...
      publicUrl = await uploadImageToBucket(
        file,
        "giro-app",
        `selfies/${data.user.id}`
      );
    } catch (uploadError) {
  ...
  const HeaderLogo = ({ showLogin = false }: { showLogin?: boolean }) => (
    ...
    >
      ...
      >
        <path
          d="M0,90 Q93,130 187,90 Q280,50 375,90"
          fill="none"
          stroke="#fff"
          strokeWidth="1.5"
        />
        <path
          d="M0,50 Q93,90 187,50 Q280,10 375,50"
          fill="none"
          stroke="#fff"
          strokeWidth="1"
        />
        <path
          d="M0,130 Q93,170 187,130 Q280,90 375,130"
          fill="none"
          stroke="#fff"
          strokeWidth="0.8"
        />
      ...
      <div
        className={`relative z-10 mb-6 ${
          showLogin ? "flex items-center justify-between" : ""
        }`}
      >
        <NextImage
      ...
      <div className="flex-1 px-6 pt-4 pb-10 bg-white overflow-y-auto">
        <div className="mb-5">
          ...
          <p className="text-gray-400 text-sm">
            Explore rotas e conquiste insígnias
          </p>
        </div>
        ...
        <form onSubmit={handleFormSubmit} className="flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <div className="relative">
              ...
              <input
                ...
                required
                className={inputClass}
                style={{ ...inputStyle("username"), paddingLeft: "2rem" }}
              />
            </div>
          </div>
          ...
          <div
            className="rounded-2xl p-4 flex items-start gap-3 mt-1"
          ...
          >
            ...
            <div>
              <p className="text-sm font-bold" style={{ color: "#830200" }}>
                Foto de referência obrigatória
              </p>
              <p className="text-xs text-orange-700/60 mt-0.5 leading-relaxed">
                Na próxima etapa você vai tirar uma selfie ou escolher uma foto
                para validar seus check-ins nas trilhas.
              </p>
            </div>
          </div>
