    ...
    await db
      ...
      .set({
        ...
        distanceKm: routeData.distanceKm
          ? routeData.distanceKm.toString()
          : null,
        estimatedMinutes: routeData.estimatedMinutes
          ? parseInt(routeData.estimatedMinutes)
          : null,
...
export async function DELETE(
  ...
  try {
    ...
    const existingSessions = await db
      .select()
      .from(routeSessions)
      .where(eq(routeSessions.routeId, id))
      .limit(1);
    ...
    if (existingSessions.length > 0) {
      return NextResponse.json(
        {
          error:
            'Não é possível excluir esta rota pois já existem aventureiros que a completaram ou iniciaram. Por favor, mude o status para "Arquivado".',
        },
        { status: 400 }
      );
    }
