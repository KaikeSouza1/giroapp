...
export async function PATCH(
  ...
  try {
    ...
    const [completedRes] = await db
      ...
      .where(
        and(
          eq(routeSessions.userId, currentSession.userId),
    ...
    if (badgeAward) {
      let [badgeObj] = await db
        .select()
        .from(badges)
        .where(eq(badges.name, badgeAward.name))
        .limit(1);
      ...
      const [alreadyHas] = await db
        ...
        .where(
          and(
            eq(userBadges.userId, currentSession.userId),
            eq(userBadges.badgeId, badgeObj.id)
          )
        )
        .limit(1);
