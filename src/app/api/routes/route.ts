...
export async function GET() {
  try {
    ...
    const allRoutes = await db
      ...
      .leftJoin(
        waypoints,
        and(eq(waypoints.routeId, routes.id), eq(waypoints.order, 1))
      )
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (
      !dbUser ||
      (dbUser.role !== "superadmin" && dbUser.role !== "admin_org")
    ) {
    ...
    const [newRoute] = await db
      ...
      .values({
        ...
        estimatedMinutes: body.estimatedMinutes
          ? parseInt(body.estimatedMinutes)
          : null,
        organizationId,
        status: "rascunho",
      })
