  ...
  try {
    ...
    const completedRoutesRes = await db
      .select({
        ...
        photos: sql<
          string[]
        >`array_remove(array_agg(${checkins.selfieImagePath}), NULL)`,
      })
      .from(routeSessions)
      ...
      .where(
        and(
          eq(routeSessions.userId, user.id),
    ...
    const badgeRules = [
      {
        count: 1,
        name: "Pioneiro",
        desc: "Concluiu a primeira rota oficial.",
        img: "internal",
      },
      {
        count: 5,
        name: "Explorador",
        desc: "Alcançou a marca de 5 rotas concluídas.",
        img: "internal",
      },
      {
        count: 10,
        name: "Lenda",
        desc: "Completou 10 rotas oficiais épicas.",
        img: "internal",
      },
    ];

    const missingRules = badgeRules.filter(
      (rule) =>
        totalCompleted >= rule.count && !currentBadgeNames.includes(rule.name)
    );

    if (missingRules.length > 0) {
      for (const rule of missingRules) {
        let [badgeObj] = await db
          .select()
          .from(badges)
