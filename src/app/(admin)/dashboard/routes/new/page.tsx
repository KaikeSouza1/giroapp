...
const routeConfig: Record<
  string,
  { osrmProfile: string; speedKmh: number; label: string }
> = {
...
export default function NewRoutePage() {
  ...
  useEffect(() => {
    ...
    async function initMap() {
      ...
      map.on("click", (e: any) => {
        ...
        setWaypoints((prev) => [
          ...prev,
          { ...newWaypoint, order: prev.length + 1 },
        ]);
  ...
  const fetchOSRMRoute = useCallback(async (wps: Waypoint[], type: string) => {
    ...
    try {
      ...
      setRouteError(
        "Não foi possível calcular a rota. Verifique os waypoints."
      );
  ...
  const drawStraightLines = useCallback(
    async (wps: Waypoint[], type: string) => {
      ...
      const durationMin = Math.round(
        (totalDist / 1000 / config.speedKmh) * 60
      ).toString();
      setForm((prev) => ({
        ...prev,
        distanceKm: distKm,
  ...
  async function handleSave() {
    ...
    if (waypoints.length === 0) {
      setError("Adicione pelo menos um waypoint no mapa.");
      return;
    }
        ...
        <div className="flex items-center justify-between px-8 py-5 bg-white border-b border-gray-100 shadow-sm z-10 relative">
          <div>
            ...
            <p className="text-gray-400 text-xs mt-0.5">
              Clique no mapa ou pesquise para adicionar waypoints
            </p>
          </div>
              ...
              {activeTab === "info" && (
                <div className="flex flex-col gap-4">
                    ...
                    {!previewUrl ? (
                      ...
                      >
                        <div className="flex flex-col items-center justify-center pt-5 pb-6">
                          <svg
                            className="w-8 h-8 mb-3 text-[#E05300]"
                            aria-hidden="true"
                          ...
                          <p className="mb-2 text-sm text-gray-600">
                            <span className="font-semibold text-[#E05300]">
                              Clique para upload
                            </span>
                          </p>
                      ...
                      <div
                        className="relative w-full rounded-2xl overflow-hidden mt-1 shadow-sm"
                        style={{
                          height: "140px",
                  ...
                  <div
                    ...
                    style={{
                      background: followRoads
                        ? "linear-gradient(135deg, #FFF4F0, #FFF9F7)"
                        : "#F7F7F7",
                  ...
                  >
                    ...
                    <button
                      ...
                      style={{
                        background: followRoads ? "#E05300" : "#D1D5DB",
                      }}
                    >
                      <span
                        className="inline-block h-4 w-4 transform rounded-full bg-white shadow-md transition-transform"
                        style={{
                          transform: followRoads
                            ? "translateX(22px)"
                            : "translateX(4px)",
                        }}
                      />
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div className="flex flex-col gap-1.5">
                      ...
                      <div className="relative">
                        ...
                        />
                        {form.distanceKm && !isRouting && (
                          <span className="absolute right-3 top-1/2 -translate-y-1/2 text-[10px] font-bold text-orange-500">
                            AUTO
                          </span>
                        )}
                      </div>
                  ...
                  {routeError && (
                    <div className="rounded-xl px-4 py-3 bg-amber-50 border border-amber-100 flex items-start gap-2">
                      ...
                      <p className="text-amber-700 text-xs leading-relaxed">
                        {routeError}. Tente mudar para modo Linha Livre ou
                        ajuste os waypoints.
                      </p>
                    </div>
                  )}
                  ...
                  {waypoints.length === 0 ? (
                    <div className="flex flex-col items-center justify-center py-8 gap-2 text-center opacity-70">
                      ...
                      <p className="text-gray-500 text-sm font-semibold mt-2">
                        Busque um local acima
                      </p>
                      <p className="text-gray-400 text-xs">
                        Ou clique diretamente no mapa ao lado
                      </p>
                    </div>
                  ) : (
                    waypoints.map((wp, i) => (
                      <div
                        key={wp.id}
                        className="rounded-2xl p-4 border border-gray-200 bg-white shadow-sm relative group hover:border-orange-300 transition-colors"
                      >
                        <div className="flex items-center justify-between mb-3">
                          <div className="flex items-center gap-2">
                            <div
                              className="w-6 h-6 rounded-full flex items-center justify-center text-xs text-white font-bold shadow-sm"
                              style={{
                                background:
                            ...
                            <span className="text-[10px] font-bold text-gray-400 bg-gray-50 px-2 py-1 rounded-md">
                              {wp.latitude.toFixed(5)},{" "}
                              {wp.longitude.toFixed(5)}
                            </span>
                          </div>
                          <button
