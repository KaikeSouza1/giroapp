...
export default function EditRoutePage() {
    ...
    async function updateMarkers() {
      ...
      waypoints.forEach((wp, i) => {
        const icon = L.divIcon({
          html: `<div style="background:linear-gradient(135deg,#830200,#E05300);color:white;width:26px;height:26px;border-radius:50% 50% 50% 0;transform:rotate(-45deg);display:flex;align-items:center;justify-content:center;font-weight:bold;font-size:10px;border:2px solid white;"><span style="transform:rotate(45deg)">${
            i + 1
          }</span></div>`,
          className: "",
          iconSize: [26, 26],
  ...
  async function handleSave() {
    ...
    if (imageFile) {
      try {
        finalImageUrl = await uploadImageToBucket(
          imageFile,
          "giro-app",
          "routes"
        );
            ...
            <div className="flex border-b border-gray-100">
              {["info", "waypoints"].map((t) => (
                <button
                  ...
                  style={{
                    color: activeTab === t ? "#E05300" : "#9CA3AF",
                    borderBottom:
                      activeTab === t ? "3px solid #E05300" : "none",
                  }}
                >
                  {t === "info"
                    ? "ℹ️ Geral"
                ...
                <div className="flex flex-col gap-5">
                  ...
                  {userRole === "superadmin" && (
                    <div className="flex flex-col gap-1.5">
                      ...
                      <select
                        ...
                        className="w-full p-3.5 rounded-2xl text-sm bg-gray-50 border border-gray-100 outline-none focus:ring-2 focus:ring-orange-500/20"
                      >
                  ...
                  <div className="flex flex-col gap-1.5">
                    ...
                    <input
                      ...
                      className="w-full p-3.5 rounded-2xl text-sm bg-gray-50 border border-gray-100 outline-none"
                    />
                  </div>

                  <div className="flex flex-col gap-1.5">
                    <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest">
                      Imagem de Capa (Bucket Supabase)
                    </label>
                        ...
                        <>
                          ...
                          <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                            <button
                              ...
                              className="bg-red-600 p-2 rounded-full text-white shadow-xl hover:bg-red-700"
                            >
                              <svg
                                width="20"
                                height="20"
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div className="flex flex-col gap-1.5">
                      ...
                      <select
                        ...
                        className="w-full p-3.5 rounded-2xl text-sm font-bold bg-orange-50 border border-orange-100 text-orange-700 outline-none"
                      >
                ...
                <div className="flex flex-col gap-4">
                  <div className="bg-gray-900 p-4 rounded-2xl shadow-xl">
                    <p className="text-[9px] font-black text-orange-500 uppercase mb-2 tracking-widest">
                      Adicionar Ponto Geográfico
                    </p>
                    <form
                      onSubmit={handleSearchAndAddWaypoint}
                      className="flex gap-2"
                    >
                      <input
                        type="text"
                        placeholder="Nome do local ou morro..."
                        value={wpSearchQuery}
                        onChange={(e) => setWpSearchQuery(e.target.value)}
                        className="flex-1 p-2.5 rounded-xl text-xs outline-none bg-white/10 text-white border border-white/10"
                      />
                      <button
                        type="submit"
                        className="px-4 bg-orange-600 text-white rounded-xl text-[10px] font-black uppercase"
                      >
                        Add
                      </button>
                    </form>
                  ...
                  {waypoints.map((wp, i) => (
                    <div
                      key={wp.id}
                      className="p-4 border border-gray-100 rounded-2xl bg-white shadow-sm hover:border-orange-200 transition-all group"
                    >
                      ...
                      <input
                        ...
                        className="w-full p-2 text-sm font-black text-gray-900 border-b border-gray-100 outline-none focus:border-orange-500"
          ...
          <div className="flex-1 relative z-0">
            <div className="absolute top-6 left-6 right-6 z-[400] flex justify-center pointer-events-none">
              <form
                ...
                className="w-full max-w-xl bg-white/90 backdrop-blur-md p-2 rounded-2xl shadow-2xl flex items-center pointer-events-auto border border-white"
              >
                ...
                <input
                  type="text"
                  placeholder="Explorar Satélite do Google..."
                  value={searchQuery}
                  onChange={(e) => setSearchQuery(e.target.value)}
                  className="flex-1 px-3 text-sm font-bold outline-none bg-transparent"
                />
                <button
                  type="submit"
                  className="px-6 py-2.5 bg-gray-900 text-white text-xs font-black uppercase rounded-xl hover:bg-black transition-all"
                >
                  Buscar
                </button>
              </form>
