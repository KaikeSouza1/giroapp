...
export default function OrganizationsPage() {
  ...
  const handleDelete = async (id: string, name: string) => {
    if (
      !window.confirm(
        `ATENÇÃO: Tem certeza que deseja excluir a organização "${name}"? Todas as rotas vinculadas a ela serão apagadas permanentemente. Isso não pode ser desfeito.`
      )
    ) {
      return;
    }
  ...
  return (
    <div className="flex min-h-screen bg-gray-50">
      ...
      <main className="flex-1 p-8">
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-gray-400 text-sm mt-0.5">
              Gerencie os clientes e parceiros do GIRO
            </p>
          </div>
          <button
        ...
        {error && (
          <div className="mb-6 rounded-xl px-4 py-3 bg-red-50 border border-red-100 text-red-500 text-sm font-medium">
            {error}
          </div>
        )}
              ...
              <tbody>
                {orgs.map((org, i) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-4">
                      {org.isActive ? (
                        <span className="inline-flex text-xs font-bold px-2.5 py-1 rounded-full bg-green-100 text-green-700">
                          Ativa
                        </span>
                      ) : (
                        <span className="inline-flex text-xs font-bold px-2.5 py-1 rounded-full bg-gray-100 text-gray-600">
                          Inativa
                        </span>
                      )}
                    </td>
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center justify-end gap-3">
                        <Link
                          href={`/dashboard/organizations/${org.id}/routes`}
                          className="text-gray-400 hover:text-orange-600 transition-colors group relative"
                          title="Ver Rotas"
                        >
                  ...
                  <div className="mt-6 pt-6 border-t border-gray-100 space-y-4">
                    <div className="mb-2">
                      ...
                      <p className="text-xs text-gray-500">
                        Credenciais para o cliente acessar o painel.
                      </p>
                    </div>
